...
import {loadPersonas,resetDecision,renderDecision,renderReview} from './strategy-ui.js';
let decisionHistory=[];
...
async function api(path,body){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),18000);try{const r=await fetch(path.replace(/^\//,''),{method:body?'POST':'GET',headers:body?{'Content-Type':'application/json'}:{},body:body?JSON.stringify(body):undefined,signal:controller.signal});let data;try{data=await r.json()}catch{throw new Error('服务暂时不可用，请稍后再试。')}if(!r.ok)throw new Error(typeof data.detail==='string'?data.detail:'请求未能完成，请重试。');return data}finally{clearTimeout(timer)}}
function buttons(){$('#persona').disabled=busy||!!session&&!state?.result;document.querySelectorAll('[data-breed]').forEach(b=>b.disabled=busy||!!session&&!state?.result);document.querySelectorAll('[data-action]').forEach(b=>b.disabled=!session||!!state?.result||busy||(b.dataset.action==='attack'&&state.player.energy<18));$('#start').disabled=busy||!!session&&!state?.result;$('#start').innerHTML=state?.result?'再斗一场 <span>→</span>':busy?'正在开盆…':'开盆斗虫 <span>→</span>';}
function render(){
...
async function act(action){if(busy||!session||state.result||(action==='attack'&&state.player.energy<18))return;busy=true;render();$('#scene-message').textContent='执草相邀，两虫试探中…';try{
 ...
 await new Promise(r=>setTimeout(r,reduced?0:850));state=next;decisionHistory.push(next.decision);
 const item=document.createElement('div');item.className='log-entry';const title=document.createElement('h3');title.textContent=`${names[entry.player]} · ${names[entry.enemy]}`;const small=document.createElement('small');small.textContent=`第 ${entry.round} 回合 · ${next.decision.mode}`;const p=document.createElement('p');p.textContent=entry.events.join(' ');const tactic=document.createElement('p');tactic.className='log-tactic';tactic.textContent=`${next.persona.name}：${next.decision.plan_label}，预判你会${names[next.decision.predicted]}。`;item.append(small,title,p,tactic);$('#log').prepend(item);while($('#log').children.length>8)$('#log').lastChild.remove();$('#log').scrollTop=0;
...
const guide=`<h2>执草入门</h2><p>选好一只蛐蛐，点击「开盆斗虫」。每回合选择一招，双方同时结算，先耗尽对方耐力者胜。</p><h3>三招相生，知进知退</h3><ul><li><b>振翅强攻</b>：消耗 18 体力，打击对方耐力；对挑逗中的对手追加 7 点伤害。</li><li><b>收须固守</b>：恢复 24 体力、3 斗志，受到的强攻伤害约为三分之一。</li><li><b>执草挑逗</b>：恢复 8 体力、19 斗志；对手固守时，额外压低其 15 斗志。</li></ul><p>斗志越高，进攻越有力；灵敏越高，越可能闪避。耐力归零即退败；第二十回合按「剩余耐力 + 斗志 × 0.2」判胜，同分为和。快捷键 1 / 2 / 3 可出招。</p><p>这是取意传统斗蟋的虚拟策略游戏，属性与规则均为游戏设计。对手会记住本局最近六次出招。Laya 同时预测你的下一招倾向并选择战术目标，结合历史统计与招式收益决定动作；模型不可用时会标明「记忆规则」。你可在开盆前选择急先锋、守拙翁或听风客，不同馆主重视的局面不同。战绩仅保存在本机浏览器。</p>`;
$('#review').onclick=()=>{modal('');renderReview($('#modal-body'),decisionHistory)};
